...
class mapOutputs:
    ...
    def getData(self, step, sl=0.):
        ...
        if self.nbPts == 0:
            ...
            self.vertices = mesh_struct[self.infoCoords]
            self.cells = mesh_struct[self.infoCells]

            Tmesh = VoroBuild()
            Tmesh.initVoronoi(self.vertices, self.cells)
        ...
        self.hFill, self.labels = filllabel(sl, nelev, self.ngbID)
    ...
    def buildUTMmesh(self, res=5000.0, nghb=3, smth=0):
        ...
        # Smooth elevation
        if smth > 0:
            z = gaussian_filter(z, sigma=smth)
        self.datafelev[:, :] = z
        self.datafRain[:, :] = raini
        self.datafEroDep[:, :] = th
